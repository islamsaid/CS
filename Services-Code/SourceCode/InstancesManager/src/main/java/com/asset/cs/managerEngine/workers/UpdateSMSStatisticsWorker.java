 ...
 */
public class UpdateSMSStatisticsWorker extends CCWorker {

    private static final String CLASS_NAME = "com.asset.cs.managerEngine.workers.UpdateStatisticsWorker";
    private MainService mainServ = new MainService();

    private int column;
    private int jobType;

    public UpdateSMSStatisticsWorker(int column, int jobType) {
        this.column = column;
...
//        CommonLogger.businessLogger.info("--------- Starting SMS UPDATE WORKER : " + this.getName() + " ----------");
        CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "Starting SMS Update Worker").build());
        String errorParams = "";
        try {
            String fixedErrorParams = "Update Column:[" + column + "]";
            Integer lastMSISDNTwoDigits = Manager.smsMSISDNSQueue.poll();
...
//                CommonLogger.businessLogger.info("Processing Last MSISDN TWO Digits=[" + lastMSISDNTwoDigits + "] by " + this.getName());
                ...
                if (jobType == EngineDefines.JOB_COPY_COLUMNS) {
                    mainServ.copyCounterToTemp(DBStruct.VFE_CS_SMS_CUST_STATISTICS.TABLE_NAME, column, lastMSISDNTwoDigits);
                } else {
                    mainServ.resetStatisticsCounterColumn(DBStruct.VFE_CS_SMS_CUST_STATISTICS.TABLE_NAME, column, lastMSISDNTwoDigits);
                }
//                CommonLogger.businessLogger.info(this.getName() + " Finished SMS Processing Last MSISDN TWO Digits=[" + lastMSISDNTwoDigits + "]  in [" + (System.currentTimeMillis() - startTime) + "] msec");
                CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "Finished SMS Processing Last MSISDN Two Digits")
                ...
                lastMSISDNTwoDigits = Manager.smsMSISDNSQueue.poll();
                errorParams = fixedErrorParams + ", Last MSISDN TWO Digits:" + lastMSISDNTwoDigits;
            }
//            CommonLogger.businessLogger.info("--------- Shutdown SMS UPDATE WORKER : " + this.getName() + " ----------");
            CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "Shutdown SMS Update Worker").build());
        } catch (Exception e) {
