 ...
 */
@ViewScoped
@ManagedBean(name = "violationReportPerCustomerReportBean")

public class ViolationReportPerCustomerReportBean implements Serializable {

    private ReportsModel reportsModel;
    private int count;
    private ArrayList<ReportsModel> reports;

    public void retrieveCountOfViolationSMS() {
        try {
            ...
            if (FacesContext.getCurrentInstance().getMessageList().size() == 0) {
                ...
                reportsModel.setCount(mainService.getViolationSMSPerCustomer(reportsModel));
            ...
            CommonLogger.errorLogger.error("Getting Caught Exception---->  for [" + ViolationReportPerCustomerReportBean.class.getName() + ".retrieveCountOfViolationSMS]", ex);
            Utility.showErrorMessage(null, ex.getErrorCode(), "in generating Report");
        } catch (Exception e) {
            CommonLogger.errorLogger.error("Getting Caught Exception---->  for [" + ViolationReportPerCustomerReportBean.class.getName() + ".retrieveCountOfViolationSMS]", e);
