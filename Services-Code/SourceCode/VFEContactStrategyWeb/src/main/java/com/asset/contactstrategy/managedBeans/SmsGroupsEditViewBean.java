 ...
 */
@ManagedBean(name = "groupsEditViewBean")
@ViewScoped
public class SmsGroupsEditViewBean implements Serializable {

    private static final String BEAN_NAME = "SmsGroupsEditViewBean";
    private static final long serialVersionUID = 1L;
    private SmsGroupWebModel selectedGroup = new SmsGroupWebModel();
    ...
    private SmsGroupWebModel oldSmsGroupWebModel;
    @ManagedProperty(value = "#{uploadAndParseFileBean}")
    private UploadAndParseFileBean uploadAndParseFileBean;

    @PostConstruct
    public void init() {
//        CommonLogger.businessLogger.debug(BEAN_NAME + " || " + "init:started.");
        CommonLogger.businessLogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "init started")
        ...
        try {
            ...
            if (!editFlag && !viewFlag) {
                FacesContext.getCurrentInstance().getExternalContext().redirect("SmsGroupsList.xhtml");
                return;
            }

        } catch (Exception e) {
            CommonLogger.errorLogger.error("Getting Caught Exception---->  for [" + SmsGroupsEditViewBean.class.getName() + ".init]", e);
...
//        CommonLogger.businessLogger.debug(BEAN_NAME + " || " + "init:ended in " + totalProcessingTime + " msec");
        CommonLogger.businessLogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "init ended")
...
//        CommonLogger.businessLogger.debug(BEAN_NAME + " || " + "checkEditParameter:started.");
        CommonLogger.businessLogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "checkEditParameter started")
                .put(GeneralConstants.StructuredLogKeys.BEAN_NAME, BEAN_NAME).build());
        Date stepStartDate = new Date();
        ...
        try {
            if (flash != null) {
                if (flash.containsKey("SelectedParameter")) {
                    this.selectedGroup = (SmsGroupWebModel) flash.get("SelectedParameter");
                    if (this.selectedGroup.getGroupType().getId() == GeneralConstants.GROUP_CUSTOMER_ELEMENT_FILTER) {
                        this.selectedGroup.setFilterList(this.groupFacade.retrieveSmsGroupFilters(this.selectedGroup.getVersionId()));
                    } else {
                        this.selectedGroup.setFilesModel(this.groupFacade.retrieveSmsGroupFiles(this.selectedGroup.getVersionId()));
                    ...
                    oldSmsGroupWebModel = new SmsGroupWebModel(selectedGroup);
                }
            ...
            CommonLogger.errorLogger.error(SmsGroupsEditViewBean.class.getName() + " || " + "Getting Caught Exception---->  for [checkEditParameter]" + ex);
        } catch (Exception e) {
        ...
        Date stepEndDate = new Date();
        long totalProcessingTime = stepEndDate.getTime() - stepStartDate.getTime();
//        CommonLogger.businessLogger.debug(BEAN_NAME + " || " + "checkEditParameter:ended in " + totalProcessingTime + " msec");
        CommonLogger.businessLogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "checkEditParameter ended")
                .put(GeneralConstants.StructuredLogKeys.BEAN_NAME, BEAN_NAME)
                .put(GeneralConstants.StructuredLogKeys.ELAPSED_TIME, totalProcessingTime).build());
    }

    public void cleanUploadedFiles() {
//        CommonLogger.businessLogger.debug(BEAN_NAME + " || " + "cleanUploadedFiles:started.");
        CommonLogger.businessLogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "cleanUploadedFiles started")
                .put(GeneralConstants.StructuredLogKeys.BEAN_NAME, BEAN_NAME).build());
        Date stepStartDate = new Date();
        try {
            ...
            CommonLogger.errorLogger.error(SmsGroupsEditViewBean.class.getName() + " || " + "Getting Caught Exception---->  for [cleanUploadedFiles]" + ex);
        } catch (Exception e) {
            CommonLogger.errorLogger.error("Getting Caught Exception---->  for [" + SmsGroupsEditViewBean.class.getName() + ".cleanUploadedFiles]", e);
            Utility.showErrorMessage(null, ErrorCodes.GENERAL_ERROR, " ");
        }
        Date stepEndDate = new Date();
        long totalProcessingTime = stepEndDate.getTime() - stepStartDate.getTime();
//        CommonLogger.businessLogger.debug(BEAN_NAME + " || " + "cleanUploadedFiles:ended in " + totalProcessingTime + " msec");
        CommonLogger.businessLogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "cleanUploadedFiles ended")
                .put(GeneralConstants.StructuredLogKeys.BEAN_NAME, BEAN_NAME)
                .put(GeneralConstants.StructuredLogKeys.ELAPSED_TIME, totalProcessingTime).build());
    }
...
//        CommonLogger.businessLogger.debug(BEAN_NAME + " || " + "editGroup:started.");
        ...
        webLog.setOperationName("Edit Sms Group");
        webLog.setPageName("SmsGroupEditView");
        webLog.setUserName(loggedInUser.getUsername());
        webLog.setStringBefore(oldSmsGroupWebModel.toString());
        groupFacade.editSmsGroup(selectedGroup, loggedInUser.getUserType());
...
//        CommonLogger.businessLogger.debug(BEAN_NAME + " || " + "loadAllCommercialElements:started.");
        CommonLogger.businessLogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "loadAllCommercialElements started")
                .put(GeneralConstants.StructuredLogKeys.BEAN_NAME, BEAN_NAME).build());
        Date stepStartDate = new Date();
        try {
            dwhElements = groupFacade.loadAllCommercialElements();
        } catch (CommonException ex) {
            CommonLogger.errorLogger.error(SmsGroupsEditViewBean.class.getName() + " || " + "Getting Caught Exception---->  for [loadAllCommercialElements]" + ex);
        } catch (Exception e) {
            CommonLogger.errorLogger.error("Getting Caught Exception---->  for [" + SmsGroupsEditViewBean.class.getName() + ".loadAllCommercialElements]", e);
            Utility.showErrorMessage(null, ErrorCodes.GENERAL_ERROR, " ");
        }

        Date stepEndDate = new Date();
        long totalProcessingTime = stepEndDate.getTime() - stepStartDate.getTime();
//        CommonLogger.businessLogger.debug(BEAN_NAME + " || " + "loadAllCommercialElements:ended in " + totalProcessingTime + " msec");
        CommonLogger.businessLogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "loadAllCommercialElements ended")
                .put(GeneralConstants.StructuredLogKeys.BEAN_NAME, BEAN_NAME)
                .put(GeneralConstants.StructuredLogKeys.ELAPSED_TIME, totalProcessingTime).build());
    }
    ...
    private void fillAttrValues() {
        try {
            ...
            CommonLogger.errorLogger.error(SmsGroupsEditViewBean.class.getName() + " || " + "Getting Caught Exception---->  for [fillAttrValues]" + ex);
        } catch (Exception e) {
            CommonLogger.errorLogger.error("Getting Caught Exception---->  for [" + SmsGroupsEditViewBean.class.getName() + ".fillAttrValues]", e);
            Utility.showErrorMessage(null, ErrorCodes.GENERAL_ERROR, " ");
        }
    ...
    public void addAttribute() {
        ...
        try {
            ...
            CommonLogger.errorLogger.error(SmsGroupsEditViewBean.class.getName() + " || " + "Getting Caught Exception---->  for [addAttribute]" + e);
        } finally {
    ...
    public void fillOperators() {
        try {
            ...
            CommonLogger.errorLogger.error(SmsGroupsEditViewBean.class.getName() + " || " + "Getting Caught Exception---->  for [fillOperators]" + ex);
        }
    }
    ...
    public void calculateAttributeVisibilityControls(int displayTypeId) {
        try {
            ...
            CommonLogger.errorLogger.error(SmsGroupsEditViewBean.class.getName() + " || " + "Getting Caught Exception---->  for [calculateAttributeVisibilityControls]" + ex);
        }
    }
...
//        CommonLogger.businessLogger.debug(BEAN_NAME + " || " + "createGroup:started.");
        ...
        Date stepStartDate = new Date();
        WebLogModel webLog = new WebLogModel();
        webLog.setOperationName("Create Sms Group");
        webLog.setPageName("SmsGroupEditView");
        webLog.setUserName(loggedInUser.getUsername());
        webLog.setStringBefore(null);
        groupFacade.addNewSmsGroup(selectedGroup, loggedInUser.getUserType());
        webLog.setStringAfter(selectedGroup.toString());
        WebLoggerFacade.insertWebLog(webLog);
        Date stepEndDate = new Date();
        long totalProcessingTime = stepEndDate.getTime() - stepStartDate.getTime();
...
//        CommonLogger.businessLogger.debug(BEAN_NAME + " || " + "approveGroup:started.");
        ...
        try {
            groupFacade.approveSmsGroup(selectedGroup);
            if (status == GeneralConstants.STATUS_PENDING_VALUE && selectedGroup.getStatusName().equals(GeneralConstants.APPROVAL_STATUS_NAME_EDIT)) {
                ...
                webLog.setOperationName("Approve sms group editing");
                webLog.setStringBefore(oldSmsGroupWebModel.toString());
                webLog.setStringAfter(selectedGroup.toString());
                ...
                webLog.setOperationName("Approve sms group creation");
                webLog.setStringBefore(null);
                webLog.setStringAfter(selectedGroup.toString());
                ...
                webLog.setOperationName("Approve sms group deletion");
                webLog.setStringBefore(oldSmsGroupWebModel.toString());
            ...
            CommonLogger.errorLogger.error(SmsGroupsEditViewBean.class.getName() + " || " + "Getting Caught Exception---->  for [approveGroup]" + ex);
            ...
            CommonLogger.errorLogger.error("Getting Caught Exception---->  for [" + SmsGroupsEditViewBean.class.getName() + ".approveGroup]", e);
            Utility.showErrorMessage(null, ErrorCodes.GENERAL_ERROR, " ");
        }
        Date stepEndDate = new Date();
        long totalProcessingTime = stepEndDate.getTime() - stepStartDate.getTime();
...
//        CommonLogger.businessLogger.debug(BEAN_NAME + " || " + "rejectGroup:started.");
        CommonLogger.businessLogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "rejectGroup started")
                .put(GeneralConstants.StructuredLogKeys.BEAN_NAME, BEAN_NAME).build());
        Date stepStartDate = new Date();
        FacesMessage curMessage = null;
        WebLogModel webLog = new WebLogModel();
        ...
        try {
            ...
            CommonLogger.errorLogger.error(SmsGroupsEditViewBean.class.getName() + " || " + "Getting Caught Exception---->  for [rejectGroup]" + ex);
            ...
            CommonLogger.errorLogger.error("Getting Caught Exception---->  for [" + SmsGroupsEditViewBean.class.getName() + ".rejectGroup]", e);
            Utility.showErrorMessage(null, ErrorCodes.GENERAL_ERROR, " ");
        }
        Date stepEndDate = new Date();
        long totalProcessingTime = stepEndDate.getTime() - stepStartDate.getTime();
//        CommonLogger.businessLogger.debug(BEAN_NAME + " || " + "approveGroup:ended in " + totalProcessingTime + " msec");
        CommonLogger.businessLogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "rejectGroup ended")
                .put(GeneralConstants.StructuredLogKeys.BEAN_NAME, BEAN_NAME)
                .put(GeneralConstants.StructuredLogKeys.ELAPSED_TIME, totalProcessingTime).build());
        FacesContext.getCurrentInstance().getExternalContext().getFlash().setKeepMessages(true);
        return ret;
...
//            CommonLogger.businessLogger.debug(BEAN_NAME + " || " + "saveGroup:started.");
            CommonLogger.businessLogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "saveGroup started")
                    .put(GeneralConstants.StructuredLogKeys.BEAN_NAME, BEAN_NAME).build());
            FacesMessage curMessage = null;
            Date stepStartDate = new Date();
            String ret = "SmsGroupsList.xhtml?faces-redirect=true";
            try {
                ...
                if (editable) {
                    ...
                    Utility.showInfoMessage(null, Constants.ITEM_EDITED, "Sms Group", this.selectedGroup.getGroupName());
                    FacesContext.getCurrentInstance().getExternalContext().getFlash().setKeepMessages(true);
                } else if (creation) {
                    createGroup();
                    //FacesContext.getCurrentInstance().getExternalContext().getFlash().put("ResetMessage", this.selectedGroup.getGroupName() + "added successfully");
                    Utility.showInfoMessage(null, Constants.ITEM_ADDED, "Sms Group", this.selectedGroup.getGroupName());
                    FacesContext.getCurrentInstance().getExternalContext().getFlash().setKeepMessages(true);
                }
                cleanUploadedFiles();
            } catch (CommonException ex) {
                CommonLogger.errorLogger.error(SmsGroupsEditViewBean.class.getName() + " || " + "Getting Caught Exception---->  for [saveGroup]" + ex);
                ...
                if ((ex.getErrorCode() == ErrorCodes.UNIQUE_PRIORITY) || (ex.getErrorCode() == ErrorCodes.UNIQUE_NAME)) {
                    if (ex.getErrorCode() == ErrorCodes.UNIQUE_PRIORITY) {
                        Utility.showErrorMessage(null, ErrorCodes.UNIQUE_PRIORITY, "Sms Group", selectedGroup.getGroupName());
                    }
                    if (ex.getErrorCode() == ErrorCodes.UNIQUE_NAME) {
                        Utility.showErrorMessage(null, ErrorCodes.UNIQUE_NAME, "Sms Group", selectedGroup.getGroupName());
                    }
                ...
                CommonLogger.errorLogger.error("Getting Caught Exception---->  for [" + SmsGroupsEditViewBean.class.getName() + ".saveGroup]", e);
                Utility.showErrorMessage(null, ErrorCodes.GENERAL_ERROR, " ");
            }

            Date stepEndDate = new Date();
            long totalProcessingTime = stepEndDate.getTime() - stepStartDate.getTime();
//            CommonLogger.businessLogger.debug(BEAN_NAME + " || " + "saveGroup:ended in " + totalProcessingTime + " msec");
            CommonLogger.businessLogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "saveGroup ended")
                    .put(GeneralConstants.StructuredLogKeys.BEAN_NAME, BEAN_NAME)
                    .put(GeneralConstants.StructuredLogKeys.ELAPSED_TIME, totalProcessingTime).build());
            return ret;
        }
...
//        CommonLogger.businessLogger.debug(BEAN_NAME + " || " + "cancelGroup:started.");
        CommonLogger.businessLogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "cancelGroup started")
                .put(GeneralConstants.StructuredLogKeys.BEAN_NAME, BEAN_NAME).build());
        Date stepStartDate = new Date();

        Date stepEndDate = new Date();
        long totalProcessingTime = stepEndDate.getTime() - stepStartDate.getTime();
//        CommonLogger.businessLogger.debug(BEAN_NAME + " || " + "cancelGroup:ended in " + totalProcessingTime + " msec");
        CommonLogger.businessLogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "cancelGroup ended")
                .put(GeneralConstants.StructuredLogKeys.BEAN_NAME, BEAN_NAME)
                .put(GeneralConstants.StructuredLogKeys.ELAPSED_TIME, totalProcessingTime).build());
        return "SmsGroupsList.xhtml?faces-redirect=true";
    }
    ...
    /////Setters and Getters/////
    /////////////////////////////
    public SmsGroupWebModel getSelectedGroup() {
        return selectedGroup;
    }

    public void setSelectedGroup(SmsGroupWebModel selectedGroup) {
    ...
    public SmsGroupFacade getGroupFacade() {
        return groupFacade;
    }

    public void setGroupFacade(SmsGroupFacade groupFacade) {
