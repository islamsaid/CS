...
//                    CommonLogger.businessLogger.info("SMSQueueSize: " + parserSenderQueue.size() + " SMSQueueRemaianingSize:" + parserSenderQueue.remainingCapacity() + " for app_id:" + mapEntry.getKey());
                    CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "SMS Queue Stats")
                     ...
                     Iterator smsReSendQueueMapiterator = Manager.deqeueurs_ReSenders_QMap.entrySet().iterator();
                while (smsReSendQueueMapiterator.hasNext()) {
                    Map.Entry mapEntry = (Map.Entry) smsReSendQueueMapiterator.next();
                    ArrayBlockingQueue<HashObject> parserSenderQueue = (ArrayBlockingQueue<HashObject>) mapEntry.getValue();
                   CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,
                           Thread.currentThread().getName() +" | App ID:" + mapEntry.getKey() + " | ReSendQMap SMSQueueSize: " + parserSenderQueue.size() + " SMSQueueRemaianingSize:" + parserSenderQueue.remainingCapacity()).build());
                }
