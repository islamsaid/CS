...
import com.asset.contactstrategy.common.defines.DBStruct.LK_QUEUES_TYPE;
...
import com.asset.cs.sendingsms.threads.EnqueuerThread;
import com.asset.cs.sendingsms.threads.MonitorThread;
import com.asset.cs.sendingsms.threads.RabbitmqDequeuer;
import com.asset.cs.sendingsms.threads.ReceivedSMSLogThread;
...
public class Manager extends EngineManager {
    ...
    public static HashMap<String, ArrayBlockingQueue<HashObject>> deqeueurs_ReSenders_QMap = new HashMap<String, ArrayBlockingQueue<HashObject>>();
    ...
    public static HashMap<String, ArrayBlockingQueue<EnqueueModel>> deliveredSMSQueuesMap = new HashMap<>();
    public static ArrayBlockingQueue<ReceivedSMSLogModel> receivedSMSLogQueue = new ArrayBlockingQueue<ReceivedSMSLogModel>(Integer.valueOf(Defines.fileConfigurations.get(Defines.RECEIVED_SMS_LOG_QUEUE_SIZE)));
    public static ExecutorService receivedSMSLogThreadPool = Executors.newFixedThreadPool(Integer.valueOf(Defines.fileConfigurations.get(Defines.RECEIVED_SMS_LOG_THRED_POOL_SIZE)));
    public static HashMap<String, ExecutorService> enqueuersPoolMap = new HashMap<String, ExecutorService>();
    ...
    public static void prepareSMPPApps() throws CommonException, IOException {
        CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,
                "Engine Starting preparing threads and queues").build());
        ...
        while (smppAppsIterator.hasNext()) {
            ...
            try {
                ...
                addAppToDeqeueursReSendersSMSMap(appQueue);
                addAppToRecieverDeliveryRespMap(appQueue);
                addAppToRecvDelMap(appQueue);

                if (com.asset.contactstrategy.common.defines.Defines.MESSAGING_MODE
                        .equals(com.asset.contactstrategy.common.defines.Defines.RABBITMQ)) {
                    startRabbitmqDequeuers(appQueue);
                } else if (com.asset.contactstrategy.common.defines.Defines.MESSAGING_MODE
                        .equals(com.asset.contactstrategy.common.defines.Defines.ORCLAQ)) {
        ...
        CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,
                "Engine finished preparing threads and queues").build());
    }
...
//            CommonLogger.businessLogger.info("Instance application queues: " + instanceKey);
     ...
     public static void startReceivedSMSLogThreads() {
        for (int i = 0; i < 2; i++) {
            ReceivedSMSLogThread receivedSmsLogThread = new ReceivedSMSLogThread(i+1);
            receivedSMSLogThreadPool.submit(receivedSmsLogThread);
        }
    }
...
//        CommonLogger.businessLogger.info("Starting creating app sms queue for APP_ID:" + appQueue.getAppName());
    ...
    private static void addAppToDeqeueursReSendersSMSMap(QueueModel appQueue) throws CommonException {
        CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,
                "Starting creating app sms queue for APP_ID:" + appQueue.getAppName()).build());
        connectionPerQueue.put(appQueue.getAppName(), new DataSourceManager(appQueue.getDatabaseUrl(), appQueue.getSchemaName(), appQueue.getSchemaPassword()));
        deqeueurs_ReSenders_QMap.put(appQueue.getAppName(), new ArrayBlockingQueue<HashObject>(Integer.valueOf(Defines.fileConfigurations.get(Defines.SENDER_SMS_QUEUE_SIZE))));
    }
...
//    }
    ...
    public static void shutdownEngine() {
        CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,
                "Manager Starting Shutdown Engine").build());
        try {
            ...
            CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,
                    "Manager Shutdown all deqeueurs Thread pools").build());
            ...
            CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,
                    "Manager Shutdown all senders Thread pools ").build());
            Defines.SENDER_THREAD_SHUTDOWN_FLAG = true;
            
            CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,
                    "Manager Shutdown all Enqueuer Thread pools").build());
            Iterator enqueuerMapiterator = enqueuersPoolMap.entrySet().iterator();
            while (enqueuerMapiterator.hasNext()) {
                Map.Entry mapEntry = (Map.Entry) enqueuerMapiterator.next();
                ExecutorService enqueuerPoolExec = (ExecutorService) mapEntry.getValue();
                enqueuerPoolExec.shutdown();
                enqueuerPoolExec.awaitTermination(Long.MAX_VALUE, TimeUnit.MILLISECONDS);
            }
            
            CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,
                    "Manager Shutdown all Enqueuer Thread pools ").build());
            
            ExecutorService receivedSMSLogPoolExec = (ExecutorService) receivedSMSLogThreadPool;
            receivedSMSLogPoolExec.shutdown();
            receivedSMSLogPoolExec.awaitTermination(Long.MAX_VALUE, TimeUnit.MILLISECONDS);
            //
...
//                CommonLogger.businessLogger.info("SMSQueue with size:" + ((ArrayBlockingQueue<HashObject>) mapEntry.getValue()).size() + " for APP_ID:" + (String) mapEntry.getKey());
            ...
            Iterator reSendSmsQueueIterator = deqeueurs_ReSenders_QMap.entrySet().iterator();
            while (reSendSmsQueueIterator.hasNext()) {
                Map.Entry mapEntry = (Map.Entry) reSendSmsQueueIterator.next();
                 CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,
                         "SMSQueue in resendQMap with size:" + ((ArrayBlockingQueue<HashObject>) mapEntry.getValue()).size() + " for APP_ID:" + (String) mapEntry.getKey()).build());
            }
            /* ISSUE 25/10, put sms objects for each session on it's blocking queue. */
...
//        CommonLogger.businessLogger.info("Engine Starting adding new APP with id:" + smppApp.getAppName());
        ...
        try {
            ...
            if (deploymentQueues.containsKey(tmpApp.getAppName())) {
                ...
                addAppToDeqeueursReSendersSMSMap(tmpApp);
...
//        CommonLogger.businessLogger.info("Manager Starting Shutdown APP queues,threadpools for APP_id: " + appId);
        try {
            if (com.asset.contactstrategy.common.defines.Defines.MESSAGING_MODE
                    .equals(com.asset.contactstrategy.common.defines.Defines.RABBITMQ)) {
                CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,
                        "Manager Starting Shutdown RabbitMQ APP queues,Connections for APP_id: " + appId).build());
                List<Channel> channels = Defines.queueChannelMap.get(Long.parseLong(appId));
                for (int i = 0; i < channels.size(); i++) {
                    Channel channel = channels.get(i);
                    channel.basicCancel(appId);
                    RabbitmqUtil.closeChannel(channel);
                }
                com.rabbitmq.client.Connection connection = Defines.queueConnectionMap.get(Long.parseLong(appId));
                RabbitmqUtil.closeConnection(connection);
            } else if (com.asset.contactstrategy.common.defines.Defines.MESSAGING_MODE
                    .equals(com.asset.contactstrategy.common.defines.Defines.ORCLAQ)) {
                CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,
                        "Manager Starting Shutdown APP queues,threadpools for APP_id: " + appId).build());
...
//            CommonLogger.businessLogger.info("smsQueue will be removed from map with size:" + deqeueurs_Senders_QMap.get(appId).size() + "for APP_id:" + appId);
            CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "SMS Queue will be Removed from map")
                    ...
                    .put(GeneralConstants.StructuredLogKeys.APP_ID, appId).build());
             CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "smsQueue will be removed from reSendMap with size:")
                    .put(GeneralConstants.StructuredLogKeys.QUEUE_SIZE, deqeueurs_ReSenders_QMap.get(appId).size())
                    .put(GeneralConstants.StructuredLogKeys.APP_ID, appId).build());
            /* ISSUE 25/10, put sms objects for each session on it's blocking queue. */
...
//            CommonLogger.businessLogger.info("StatusCountHSize: " + Manager.deliverResp_updateStatusCountH_Q.size() + " StatusCountHSizeRemaianingSize:" + Manager.deliverResp_updateStatusCountH_Q.remainingCapacity());
            ...
            deqeueurs_ReSenders_QMap.remove(appId);
...
//            CommonLogger.businessLogger.info("Manager finished shutdown for APP_id:" + appId);
    ...
    public static void initLivenessThread() {
        CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,
                "JobManager.initLivenessThread() Invoked...").build());
        ...
        CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,
                "JobManager.initLivenessThread() Ended...").build());
    }

    private static void startRabbitmqDequeuers(QueueModel app) throws CommonException, IOException {
        CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,
                "Starting creating RabbitMQ dequeuer for APP_ID:" + app.getAppName()).build());
        MainService commonMainService = new MainService();
        com.rabbitmq.client.Connection conn = RabbitmqUtil.getConnection();
        Defines.queueConnectionMap.put(app.getAppId(), conn);
        for (int i = 0; i < app.getDequeuePoolSize(); i++) {
            ...
            if (!app.isTimeWindowFlag()
                    || (app.isTimeWindowFlag()
                    && systemTime >= ((app.getTimeWindowFromHour() * 100) + app.getTimeWindowFromMin())
                    && systemTime < ((app.getTimeWindowToHour() * 100) + app.getTimeWindowToMin()))) {
                Channel channel = null;
                try {
                    channel = RabbitmqUtil.getChannel(conn);
                    channel.basicQos(app.getThreshold());
                } catch (IOException ex) {
                    throw new CommonException("Exception --> Failed to create RabbitMQ channel ",
                            com.asset.contactstrategy.common.defines.ErrorCodes.RABBITMQ.CHANNEL_ERROR);
                }
                commonMainService.registerRabbitmqConsumer(channel, new RabbitmqDequeuer(channel, app), app, "");
                if (Defines.queueChannelMap.get(app.getAppId()) == null) {
                    Defines.queueChannelMap.put(app.getAppId(), new ArrayList<Channel>());
                }
                Defines.queueChannelMap.get(app.getAppId()).add(channel);
            }
        }
    }

    public static void startEnqueuerThreads() {
        QueueModel appQueue;
        for (Map.Entry pair : Defines.appQueueMap.entrySet()) {
            try {
                appQueue = (QueueModel) ((QueueModel) pair.getValue()).clone();
                if (((!Defines.CLOUD_MODE) && (!deploymentQueues.containsKey(appQueue.getAppName()))) || (appQueue.getQueueType() != LK_QUEUES_TYPE.SMS_SENDER_QUEUES)) {
                    continue;
                }
                deliveredSMSQueuesMap.put(appQueue.getAppName(), new ArrayBlockingQueue<EnqueueModel>(Integer.valueOf(Defines.fileConfigurations.get(Defines.RECIEVED_SMS_QUEUE_SIZE))));
                CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,
                        "Starting creating enqueuer thread pool for queue:" + appQueue.getAppName()).build());
                int enqueuerThreadPoolSize = Integer.valueOf(Defines.fileConfigurations.get(Defines.RECEIVED_SMS_LOG_THRED_POOL_SIZE));
                ExecutorService enqueuerPool = Executors.newFixedThreadPool(enqueuerThreadPoolSize * 2);
                for (int i = 0; i < enqueuerThreadPoolSize; i++) {
                    EnqueuerThread deliverSmEnqueuerThread = new EnqueuerThread(deliveredSMSQueuesMap.get(appQueue.getAppName()),
                            Defines.databaseConfigurations.get(Defines.DELIVER_QUEUE_NAME_PREFIX), appQueue.getAppName(), i + 1);
                    enqueuerPool.submit(deliverSmEnqueuerThread);
                }
                enqueuersPoolMap.put(appQueue.getAppName(), enqueuerPool);

            } catch (CloneNotSupportedException ex) {
                CommonLogger.errorLogger.error("CloneNotSupportedException in startEnqueuerThreads || " + ex, ex);
            }
        }
    }
