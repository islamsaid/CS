...
public class SenderThread extends Thread {
    ...
    private ArrayBlockingQueue<HashObject> deqeuer_reSender_Q;
    ...
    public SenderThread(QueueModel appQueue, int threadNumber) {
        ...
        this.deqeuer_reSender_Q = Manager.deqeueurs_ReSenders_QMap.get(appQueue.getAppName());
...
//        CommonLogger.businessLogger.info("*****************SENDER THREAD STARTED FOR APP_ID:" + this.appQueue.getAppName() + "***********************");
        ...
        while (!Defines.DEQUEUER_THREAD_SHUTDOWN_FLAG && !Manager.appsThreadsShutdownMap.get(this.appQueue.getAppName())) {
            try {
                  if (Defines.RELOAD_SMSCS_FLAG) {
                    this.appQueue = Defines.appQueueMap.get(this.appQueue.getAppName());
//                    smppSession.updateIPArray(this.appQueue);
                    Defines.RELOAD_SMSCS_FLAG = false;
                }
                smsObject = deqeuer_reSender_Q.poll();
                if (smsObject == null) {
                    ...
                    if (smppSession.getConnection() != null && smppSession.getConnectionStatus() == true) {
                        ...
                        try {
                            ...
                            CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,
                                    "S || " + this.appQueue.getAppName() + "  || submit_sm || " + submitReturnValue).build());
//                            CommonLogger.businessLogger.debug("S || " + this.appQueue.getAppName() + "  || submit_sm || " + submitReturnValue);
                        ...
                        if (submitReturnValue == 1) { // message written successfully on smsc socket, should be enqueued to update delivery counter.
                            ...
                            CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,
                                    "Enqueuing message again for resending submitReturnValue != 1").build());
                            CommonLogger.businessLogger.info(Thread.currentThread().getName() + " | Socket Connection Failure to smsc | Enqueuing message again in queue: " + appQueue.getAppName() + "for resending");
                            if (com.asset.contactstrategy.common.defines.Defines.MESSAGING_MODE.equals(com.asset.contactstrategy.common.defines.Defines.RABBITMQ)) {

                                reenqueueSMS(smsObject, appQueue.getAppName());
                            } else if (com.asset.contactstrategy.common.defines.Defines.MESSAGING_MODE.equals(com.asset.contactstrategy.common.defines.Defines.ORCLAQ)) {
                                mainService.enqMsg(smsObject, Manager.connectionPerQueue.get(appQueue.getAppName()));
                            }
                            CommonLogger.businessLogger.debug(Thread.currentThread().getName() + " | SMS Enqueued again in queue: " + appQueue.getAppName());
                            CommonLogger.businessLogger.debug("MSG ENQD");
                        }
                    } else//enq message again in case not connected to SMSC
                    {
                        CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,
                                "Enqueuing message again in database queue for resending not connected to smsc").build());
                        if (com.asset.contactstrategy.common.defines.Defines.MESSAGING_MODE.equals(com.asset.contactstrategy.common.defines.Defines.RABBITMQ)) {
                            reenqueueSMS(smsObject, appQueue.getAppName());
                        } else if (com.asset.contactstrategy.common.defines.Defines.MESSAGING_MODE.equals(com.asset.contactstrategy.common.defines.Defines.ORCLAQ)) {
                            mainService.enqMsg(smsObject, Manager.connectionPerQueue.get(appQueue.getAppName()));
...
//                        CommonLogger.businessLogger.info("SLEEP || " + waitTime + " || SendRcvHash size ||" + this.smppSession.getSendRcvHash().size());
                    ...
                    CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,"AWAKE").build());
                    msgCounter = 0;
...
//                    smppSession.updateIPArray(this.appQueue);
                ...
                if (smsObject != null) {
                    ...
                    if (smppSession.getConnection() != null && smppSession.getConnectionStatus() == true) {
                        ...
                        try {
                            submitReturnValue = smppSession.Submit_SM(Data.GSM_TON_INTERNATIONAL, Data.GSM_NPI_ISDN, smsObject);
                            CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,"S || " + this.appQueue.getAppName() + "  || submit_sm || " + submitReturnValue).build());
                            CommonLogger.businessLogger.debug("S || " + this.appQueue.getAppName() + "  || submit_sm || " + submitReturnValue);
                        } catch (Exception ex) {//added in case connection on port is true but username and password are wrong
                            CommonLogger.businessLogger.error("SenderThread Caugth Exception in Submit--->" + ex);
                            CommonLogger.errorLogger.error("SenderThread Caugth Exception in Submit--->" + ex, ex);
                    ...
                    {
                        CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,
                                "Enqueuing message again for resending not connected to smsc").build());
                        if (com.asset.contactstrategy.common.defines.Defines.MESSAGING_MODE.equals(com.asset.contactstrategy.common.defines.Defines.RABBITMQ)) {
                            reenqueueSMS(smsObject, appQueue.getAppName());
                        } else if (com.asset.contactstrategy.common.defines.Defines.MESSAGING_MODE.equals(com.asset.contactstrategy.common.defines.Defines.ORCLAQ)) {
                            mainService.enqMsg(smsObject, Manager.connectionPerQueue.get(appQueue.getAppName()));
...
//        CommonLogger.businessLogger.info("*****************SNEDER THREAD FINISHED FOR APP_ID:" + this.appQueue.getAppName() + "***********************");
    ...
    private void reenqueueSMS(HashObject smsObject, String queueName) throws IOException, CommonException {
        com.asset.contactstrategy.common.service.RabbitmqQueueService rabbitmqQueueService
                = new com.asset.contactstrategy.common.service.RabbitmqQueueService();
        ArrayList<EnqueueModelREST> smsPatch = new ArrayList<>();
        EnqueueModelREST enqModelRest = null;
        try {
            enqModelRest = com.asset.contactstrategy.common.utils.Utility.mapSMSToEnqueueModelRest(smsObject.getSMS());
        } catch (SQLException ex) {
            CommonLogger.errorLogger.error(ex);
        }
        smsPatch.add(enqModelRest);
        rabbitmqQueueService.enqueueBatch(smsPatch, queueName);
    }
}
