...
import com.asset.cs.sendingsms.models.AddressModel;
import com.asset.cs.sendingsms.models.DeliverSMModel;
import com.asset.cs.sendingsms.models.HeaderModel;
import com.asset.cs.sendingsms.models.ReceivedSMSLogModel;
import com.asset.cs.sendingsms.models.TLVModel;
...
//            debuglogger.info("submit_sm || " + SMSObject.getSMS().getSeqId() + " || " + smppApp.getAppName() + " || " + sequenceNumber + " || " + returnValue);
            // debuglogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "submit_SM")
            //         .put(GeneralConstants.StructuredLogKeys.ID, SMSObject.getSMS().getSeqId())
            //         .put(GeneralConstants.StructuredLogKeys.APP_NAME, smppApp.getAppName())
            //         .put(GeneralConstants.StructuredLogKeys.SEQUENCE_NUMBER, returnValue).build());
...
//    }
    ...
    public boolean ParsePDU(BufferByte DataResp) {
        ...
        headerModel.setCommandLength(header.commandLength);
        headerModel.setCommandID(header.commandID);
        headerModel.setCommandStatus(header.commandStatus);
        headerModel.setSequenceNumber(header.sequenceNumber);
        ...
        if (Integer.parseInt(valueCommand_id) == Data.DELIVER_SM) {
            debuglogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "Starting DELIVER_SM request parsing...").build());
            DeliverSMModel deliverSMModel = new DeliverSMModel(headerModel);
            deliverSMModel.setServiceType(DataResp.removeCString());
            deliverSMModel.setSource(new AddressModel(DataResp));
            deliverSMModel.setDestination(new AddressModel(DataResp));
            deliverSMModel.setEsmClass(DataResp.removeByte());
            deliverSMModel.setProtocolId(DataResp.removeByte());
            deliverSMModel.setPriorityFlag(DataResp.removeByte());
            deliverSMModel.setScheduleDeliveryTime(DataResp.removeCString());
            deliverSMModel.setValidityPeriod(DataResp.removeCString());
            deliverSMModel.setRegisteredDelivery(DataResp.removeByte());
            deliverSMModel.setReplaceIfPresentFlag(DataResp.removeByte());
            deliverSMModel.setDataCoding(DataResp.removeByte());
            deliverSMModel.setSmDefaultMsgId(DataResp.removeByte());
            deliverSMModel.setSmLength(DataResp.removeByte());

            debuglogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "source_addr_ton || 0x" + deliverSMModel.getSource().getTon()).build());
            debuglogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "source_addr_npi|| 0x" + deliverSMModel.getSource().getNpi()).build());
            debuglogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "source_addr || 0x" + deliverSMModel.getSource().getAddress()).build());
            debuglogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "dest_addr_ton || 0x" + deliverSMModel.getDestination().getTon()).build());
            debuglogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "dest_addr_npi || 0x" + deliverSMModel.getDestination().getNpi()).build());
            debuglogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "destination_addr || 0x" + deliverSMModel.getDestination().getAddress()).build());
            debuglogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "esm_classd || 0x" + deliverSMModel.getEsmClass()).build());
            debuglogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "protocol_id || 0x" + deliverSMModel.getProtocolId()).build());
            debuglogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "priority_flag || 0x" + deliverSMModel.getPriorityFlag()).build());
            debuglogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "schedule_delivery_time || 0x" + deliverSMModel.getScheduleDeliveryTime()).build());
            debuglogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "validity_period || 0x" + deliverSMModel.getValidityPeriod()).build());
            debuglogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "registered_delivery || 0x" + deliverSMModel.getRegisteredDelivery()).build());
            debuglogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "replace_if_present_flag || 0x" + deliverSMModel.getReplaceIfPresentFlag()).build());
            debuglogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "data_coding|| 0x" + deliverSMModel.getDataCoding()).build());
            debuglogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "sm_default_msg_id || 0x" + deliverSMModel.getSmDefaultMsgId()).build());
...
//            debuglogger.debug("sm_default_msg_id || 0x" + DataResp.removeByte());
            ...
            try {
                ...
                deliverSMModel.setShortMessage(latestMsg);
...
//                    debuglogger.info("Starting reading optional parameters");
                    ...
                    short decimalTag = DataResp.removeShort();
...
//                        debuglogger.info("Starting reading message state tag with value:" + status);
                        ...
                        tlvModel = new TLVModel(decimalTag, length, status);
...
//                            debuglogger.info("Starting reading message id tag with value:" + optionalId);
                            ...
                            tlvModel = new TLVModel(decimalTag, length, optionalId);
                        } catch (UnsupportedEncodingException ex) {
                            debuglogger.error("UnsupportedEncodingException in getting message from deliver_sm||  " + ex);
                            errlogger.error("UnsupportedEncodingException in getting message from deliver_sm ||  " + ex, ex);
                        }
...
//                            debuglogger.info("Reading parameter  with value:" + DataResp.removeString(length, Data.ENC_ASCII));
                            ...
                            String value = DataResp.removeString(length, Data.ENC_ASCII);
                            tlvModel = new TLVModel(decimalTag, length, value);
                        } catch (UnsupportedEncodingException ex) {
                            debuglogger.error("UnsupportedEncodingException in getting message from deliver_sm||  " + ex);
                            errlogger.error("UnsupportedEncodingException in getting message from deliver_sm ||  " + ex, ex);
                        }
                    }
                    deliverSMModel.getTlvs().put(tlvModel.getTag(), tlvModel);
...
//                debuglogger.info("Converting decimal message id: " + id + " to hexa || " + Integer.toHexString(Integer.valueOf(id)));
                try {
                    ...
                    errlogger.error("Error in converting message_id to HexString" + e, e);
                }
            }
            ...
            try {
                ...
                if (SMSObject == null) {
                    ...
                    SMSObject.getSMS().setDstMsisdn(deliverSMModel.getSource().getAddress()); //in deliver_sm source addess is the destnation msisdn of SMS.
...
//                infologger.info("deliver_sm || " + SMSObject.getSMS().getSeqId() + " || 0x" + latestMsg);
            ...
            try {
                enqueueDelivery(deliverSMModel);
            } catch (InterruptedException ex) {
                errlogger.error("Exception in deliver_sm enqueueing in rabbitmq ||  " + ex, ex);
            } catch (Exception ex) {
                errlogger.error("Exception in deliver_sm enqueueing in rabbitmq ||  " + ex, ex);
            }
        } else {
...
//                        debuglogger.info("NOT FOUND IN SENDRECV HASH || " + PDUHeader.sequenceNumber);
                        debuglogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "submit_sm_resp Not Found in SENDRECV Hash")
                                .put(GeneralConstants.StructuredLogKeys.SEQUENCE_NUMBER, PDUHeader.sequenceNumber).build());
...
//                                + " || hexaDec msgID || " + PDUHeader.retVal);
                        debuglogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "submit_sm_resp not found in SENDRECV HASH")
                                .put(GeneralConstants.StructuredLogKeys.PDU_HEADER, PDUHeader.sequenceNumber)
                                .put(GeneralConstants.StructuredLogKeys.STATUS, Integer.toHexString(PDUHeader.commandStatus).toUpperCase())
                                .put(GeneralConstants.StructuredLogKeys.MSG_ID, PDUHeader.retVal).build());
...
//                        debuglogger.info(SMSObject.SMS.getAppId() + " || " + PDUHeader.sequenceNumber);
                        debuglogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "submit_sm_resp Success")
                                .put(GeneralConstants.StructuredLogKeys.APP_ID, SMSObject.SMS.getAppId())
                                .put(GeneralConstants.StructuredLogKeys.SEQUENCE_NUMBER, PDUHeader.sequenceNumber).build());
//                        debuglogger.debug("submit_sm_resp || " + SMSObject.SMS.getAppId() + " || " + PDUHeader.sequenceNumber + " || SUCCESS || "
//                                + Integer.toHexString(PDUHeader.commandStatus).toUpperCase()
//                                + " || hexaDec msgID || " + PDUHeader.retVal + " || " + conversionRequired + " Decimal msgID || " + decimalRetVal + " || hexaDec trimmedMsgID || " + SMSObject.SMS.getSmsc_msg_id_trimmed() + " || " + SMSObject.log + " || QUEUE_MSG_ID || " + SMSObject.getMsgid() + " || BATCH_ID|| " + SMSObject.getBatchId());
                        debuglogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "submit_sm_resp success")
                                .put(GeneralConstants.StructuredLogKeys.APP_ID, SMSObject.SMS.getAppId())
                                .put(GeneralConstants.StructuredLogKeys.STATUS, Integer.toHexString(PDUHeader.commandStatus).toUpperCase())
                                .put(GeneralConstants.StructuredLogKeys.HEX_MSG_ID, PDUHeader.retVal)
                                .put(GeneralConstants.StructuredLogKeys.CONVERSION_REQUIRED, conversionRequired)
...
//                            debuglogger.info(SMSObject.SMS.getAppId() + " || " + PDUHeader.sequenceNumber);
                            debuglogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "submit_sm_resp Fail")
                                    .put(GeneralConstants.StructuredLogKeys.APP_ID, SMSObject.SMS.getAppId())
                                    .put(GeneralConstants.StructuredLogKeys.SEQUENCE_NUMBER, PDUHeader.sequenceNumber).build());
...
//                                    + " || " + SMSObject.log + " || QUEUE_MSG_ID || " + SMSObject.getMsgid() + " || BATCH_ID|| " + SMSObject.getBatchId());
                            debuglogger.debug(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "submit_sm_resp FAIL")
                                    .put(GeneralConstants.StructuredLogKeys.APP_ID, SMSObject.SMS.getAppId())
                                    .put(GeneralConstants.StructuredLogKeys.SEQUENCE_NUMBER, PDUHeader.sequenceNumber)
                                    .put(GeneralConstants.StructuredLogKeys.STATUS, Integer.toHexString(PDUHeader.commandStatus).toUpperCase())
                                    .put(GeneralConstants.StructuredLogKeys.QUEUE_MSG_ID, SMSObject.getMsgid())
...
//                            mainService.enqMsg(SMSObject, Manager.connectionPerQueue.get(this.smppApp.getAppName()));
                            Manager.deqeueurs_ReSenders_QMap.get(this.smppApp.getAppName()).put(SMSObject);
                        } else {
...
//                                    + " || Message = " + SMSObject.SMS.getMsgTxt() + " || QUEUE_MSG_ID || " + SMSObject.getMsgid() + " || BATCH_ID|| " + SMSObject.getBatchId());
                            debuglogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, SMSObject.SMS.getMsgTxt())
                                    .put(GeneralConstants.StructuredLogKeys.APP_ID, SMSObject.SMS.getAppId())
                                    .put(GeneralConstants.StructuredLogKeys.SEQUENCE_NUMBER, PDUHeader.sequenceNumber)
                                    .put(GeneralConstants.StructuredLogKeys.COMMAND_STATUS, Integer.toHexString(PDUHeader.commandStatus).toUpperCase())
...
//                                    + " || SarRefNum = " + SMSObject.SMS.getConcSarRefNum().intValue()
//                                    + " || Message = " + SMSObject.SMS.getMsgTxt() + " || QUEUE_MSG_ID || " + SMSObject.getMsgid() + " || BATCH_ID|| " + SMSObject.getBatchId());
                            infologger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "submit_sm_resp Fail. " + SMSObject.SMS.getMsgTxt())
...
//                .put(GeneralConstants.StructuredLogKeys.STATE, state).build());
    ...
    private BufferByte ConstructDeliverPDU(int deliversmseqnum, String messageid) {
     ...
     public void enqueueDelivery(DeliverSMModel deliverSMModel) throws InterruptedException {
        String transId = Utility.generateTransId("Deliver_");
        debuglogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE,"TransId [" + transId + "]" 
            + " Parsed delivery Request: " + deliverSMModel.toString()).build());
        EnqueueModel enqueueModel = convertDeliverSMModelTOEnqueueModel(deliverSMModel, transId);
        Manager.deliveredSMSQueuesMap.get(this.getSmppApp().getAppName()).put(enqueueModel);
        ReceivedSMSLogModel logModel = new ReceivedSMSLogModel(deliverSMModel, null,
                 new Timestamp(System.currentTimeMillis()), transId);
         Manager.receivedSMSLogQueue.add(logModel);
    }
     
     private EnqueueModelDeliverSM convertDeliverSMModelTOEnqueueModel(DeliverSMModel deliverSMModel, String transId) {
        EnqueueModelDeliverSM enqueueModel = new EnqueueModelDeliverSM(
                deliverSMModel.getProtocolId(), 
                deliverSMModel.getPriorityFlag(), 
                deliverSMModel.getScheduleDeliveryTime(), 
                deliverSMModel.getValidityPeriod(),
                deliverSMModel.getReplaceIfPresentFlag(), 
                deliverSMModel.getSmDefaultMsgId(), 
                deliverSMModel.getSmLength(), 
                deliverSMModel.getShortMessage(),
                deliverSMModel.getServiceType(),
                deliverSMModel.getSource().getTon(),
                deliverSMModel.getSource().getNpi(),
                deliverSMModel.getSource().getAddress(),
                deliverSMModel.getDestination().getTon(),
                deliverSMModel.getDestination().getNpi(),
                deliverSMModel.getDestination().getAddress(),
                deliverSMModel.getEsmClass(),
                deliverSMModel.getRegisteredDelivery(),
                deliverSMModel.getDataCoding(),
                transId
        );

        HashMap<Short, TLVOptionalModel> tlvs = new HashMap<>();

        for (Map.Entry<Short, TLVModel> entry : deliverSMModel.getTlvs().entrySet()) {
             tlvs.put(entry.getKey(), new TLVOptionalModel(entry.getValue().getTag(),
                     entry.getValue().getLength(), entry.getValue().getType(), entry.getValue().getValue() + ""));
        }
        enqueueModel.setTlvs(tlvs);
