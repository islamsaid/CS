 ...
 * @author esmail.anbar
 */
public class DeliveryBatchUpdater extends RollingBatchExecutor<FullSms, FullSms> {

    public DeliveryBatchUpdater(int i, ConsumePolicy cp, ThreadPoolExecutor tpe) {
        super(i, cp, tpe);
    }
    ...
    protected void consumeBatchList(ArrayList<FullSms> batchList) {
        ...
        try {
            Thread.currentThread().setName(originalThreadName + "-" + DeliveryBatchUpdater.class.getSimpleName());
            DeliveryFacade.updateToDelivered(batchList);
            CommonLogger.businessLogger.info(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "Updated Sms Batch to Delivered")
            ...
            CommonLogger.errorLogger.error(StructuredLogFactory.put(GeneralConstants.StructuredLogKeys.MESSAGE, "Error while Updating Sms Batch to Delivered")
                    .put(GeneralConstants.StructuredLogKeys.ELAPSED_TIME, (System.currentTimeMillis() - time))
