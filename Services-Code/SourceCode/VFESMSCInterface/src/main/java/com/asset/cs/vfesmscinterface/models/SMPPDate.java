...
public class SMPPDate implements java.io.Serializable {
	...
	private static final String FORMAT = "{0,number,00}{1,number,00}{2,number,00}{3,number,00}{4,number,00}"
			+ "{5,number,00}{6,number,0}{7,number,00}{8}";

	private static final String SHORT_FORMAT = "{0,number,00}{1,number,00}{2,number,00}{3,number,00}{4,number,00}"
			+ "{5,number,00}";
	...
	/**
	 * Create a new SMPPDate representing an absolute time. The timezone offset will
	 * be set according to the current default timezone (as determined by the JVM).
	 * 
	 * @param d
	...
	/**
	 * Get a calendar object representing the internal time of this SMPPDate. This
	 * method should not be used to retrieve a calendar from relative time
	 * specifications. That is, if <code>this.isRelative() == true</code>, you
	 * should not attempt to use the object returned from this method.
	 */
	...
	/**
	 * Get the month, or number of months in a relative time spec. January is month
	 * 1.
	 */
	public int getMonth() {
		return month;
	...
	/**
	 * Get the day, or number of days in a relative time spec. Day is in the range
	 * [1..31]
	 */
	public int getDay() {
		return day;
	...
	/**
	 * Get the minute, or number of minutes in a relative time spec. Minute is in
	 * the range [00..59]
	 */
	...
	/**
	 * Get the second, or number of seconds in a relative time spec. Second is in
	 * the range [00..59]
	 */
	...
	/**
	 * Get the tenths of a second. Always zero in a relative time spec. Tenths is in
	 * the range [0..9]
	 */
	public int getTenth() {
		return tenth;
	 ...
	 * value is always positive. Use {@link #getSign} to determine if the time is
	 * ahead of or behind UTC. utcOffset is in the range [0..48]
	 */
	...
	/**
	 * Get the timezone that this date is in. If this object represents a relative
	 * time definition, then this method will return <code>null
	 * </code>.
	 * 
	...
	/**
	 * Get the UTC offset qualifier. This flag is '+' to indicate that the time spec
	 * is ahead of UTC or '-' to indicate it is behind UTC. If the time spec is a
	 * relative time spec, this flag will be 'R'.
	 * 
	 * @see #getUtcOffset
	 */
	public char getSign() {
	...
	/**
	 * Test if this SMPPDate has timezone information associated with it. Relative
	 * time specs have no timezone information, neither does the short
	 * (12-character) form of the absolute time spec. The short-form absolute format
	 * should only be used by an SMSC - applications should never create a
	 * short-form format to send to the SMSC.
	 * 
	 * @return
	 */
	...
	public boolean equals(Object obj) {
		if (obj instanceof SMPPDate) {
			...
			int diff = (year - d.year) + (month - d.month) + (day - d.day) + (hour - d.hour) + (minute - d.minute)
					+ (second - d.second) + (tenth - d.tenth) + ((int) sign - (int) d.sign);
			boolean sameTz = savedTimeZone == null ? d.savedTimeZone == null : savedTimeZone.equals(d.savedTimeZone);
	...
	/**
	 * Parse an SMPP date string. This method returns a handle to a newly created
	 * SMPPDate object with its fields initialised using the values in the
	 * SMPP-formatted date string <code>s</code>.
	 *
		...
		try {
			...
			if (longForm) {
				...
				if (d.sign != 'R') {
					...
					Object[] args = new Object[] { new Character(d.sign), Integer.valueOf(hours),
	...
	public String toString() {
		...
		Object[] args = { new Integer(year), new Integer(month), new Integer(day), new Integer(hour),
				new Integer(minute), new Integer(second), new Integer(tenth), new Integer(utcOffset),
				new Character(sign), };
